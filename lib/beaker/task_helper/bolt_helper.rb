...
module Beaker
  module TaskHelper
    ...
    module BoltHelper
      ...
      # @param [String] source where to retrieve gem from, gem source or path to locally built gem
      #
      ...
      def install_bolt_on(hosts, version = BOLT_VERSION, source = nil)
        Array(hosts).each do |host|
          ...
          raise message unless result.stdout =~ %r{Usage: bolt <subcommand>}
        end
      end
      ...
      def run_bolt_task(task_name:, params: nil, password: DEFAULT_PASSWORD, host: 'localhost', format: 'human', module_path: DEFAULT_MODULE_PATH) # rubocop:disable Metrics/LineLength
        ...
        # windows is special
        bolt_full_cli << ' --transport winrm --user Administrator' if default['platform'] =~ %r{windows}
  ...
  iex ((New-Object System.Net.WebClient).DownloadString($choco_install_uri))
        ...
        raise message unless result.stdout =~ %r{ruby 2}
      end
      ...
      def verify_and_copy_gem(host, source)
        ...
        raise ArgumentError message unless source =~ %r{\.gem$}
